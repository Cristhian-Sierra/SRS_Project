...
class Filter_searchDAO{
	...
	function select() {
		return "select distinct idFilter_search, search_date, search_time, hindex_filter, references_filter, country_filter, category_filter, area_filter, quartile_filter, sjr_filter
				from filter_search
				where idFilter_search = '" . $this -> idFilter_search . "'";
	}
	...
	function selectAllCo() {
		return "select distinct co.name, count(fs.country_filter) as countc from filter_search as fs,country as co where fs.country_filter=co.idCountry group by co.name";
	}

	function selectAllA() {
		return "select distinct a.name, count(fs.area_filter) as counta from filter_search as fs,area as a WHERE fs.area_filter=a.idArea group by a.name";
	}

	function selectAllCa() {
		return "select distinct ca.name, count(fs.category_filter) as countca from filter_search as fs,category as ca where fs.category_filter=ca.idCategory group by
		 ca.name";
	}
	function selectAllDate() {
		return "select distinct search_date as day,count(idFilter_search) from filter_search where search_date=search_date group by day";
	}

	function selectAllN() {
		return "select distinct fs.idFilter_search, fs.search_date, fs.search_time, fs.hindex_filter, fs.references_filter, co.name as country, ca.name as category,a.name as area, fs.quartile_filter, fs.sjr_filter
		...
		UNION
		select distinct fs.idFilter_search, fs.search_date, fs.search_time, fs.hindex_filter, fs.references_filter, fs.country_filter,fs.category_filter,a.name as area, fs.quartile_filter, fs.sjr_filter
		from filter_search as fs, area as a 
		where  
		fs.area_filter= a.idArea and fs.country_filter='' AND fs.category_filter=''
		UNION
		select distinct fs.idFilter_search, fs.search_date, fs.search_time, fs.hindex_filter, fs.references_filter, fs.country_filter ,a.name as area ,ca.name as category, 
		...
		UNION
		select distinct fs.idFilter_search, fs.search_date, fs.search_time, fs.hindex_filter, fs.references_filter, co.name as country, fs.category_filter,a.name as area, fs.quartile_filter, fs.sjr_filter
		...
		UNION
		select distinct fs.idFilter_search, fs.search_date, fs.search_time, fs.hindex_filter, fs.references_filter, co.name as country, fs.category_filter,fs.area_filter, fs.quartile_filter, fs.sjr_filter
		from filter_search as fs, country as co 
		where  
		fs.country_filter=co.idCountry  AND fs.area_filter= '' AND fs.category_filter=''
		UNION
		select distinct fs.idFilter_search, fs.search_date, fs.search_time, fs.hindex_filter, fs.references_filter, fs.country_filter, fs.category_filter,fs.area_filter, fs.quartile_filter, fs.sjr_filter
		from filter_search as fs, country as co 
		where  
